...
## 実装前の差分確認（2026-07-30確定）

UI・文言・色などの修正を依頼された際は、実装に着手する前に必ず該当ファイルの現在の内容を確認する。過去のやり取りで既に同じ内容が実装・commit済みの場合は、重複して同じ変更を加えない。その場合は「差分なし・実装済み」である旨と、該当するcommitを報告するだけでよく、空コミットやbuildのやり直しを目的化しない（tsc/next buildなど下記の確認自体は通常どおり実施してよい）。
